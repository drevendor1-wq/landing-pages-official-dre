...
useEffect(() => {
    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 ${
      ...
      <div className={`mx-auto px-6 transition-all duration-700 ${
        ...
        <div className={`flex items-center justify-between px-6 py-3 transition-all duration-500 ${
          isScrolled 
            ? 'bg-white/80 backdrop-blur-md shadow-2xl border border-white/20 rounded-full gap-2 py-1' 
            : 'bg-transparent gap-4 py-4'
        }`}>
          {/* Logo */}
 ...
 <div className="block">
  ...
  className="bg-[#B35109] text-white hover:bg-zinc-900 px-2 py-1 text-[6px] font-bold rounded-full transition-all uppercase sm:px-4 sm:py-1.5 sm:text-[7px] whitespace-nowrap"
>
  REQUEST BROCHURE
</button>
    </div>
