...
const Navbar = () => {
  ...
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent("Hello, I am interested in ZED SHEIKH ZAYED by Ora Developer Project. Please provide more information regarding the project and available units.")}`;
  ...
  return (
    <section>
      <motion.nav 
        ...
        className="fixed top-0 left-0 w-full z-[100] transition-all duration-500 ease-out border-b border-white/5"
      >
        <div className="max-w-[1400px] mx-auto px-8 flex items-center justify-between">
          ...
          {/* DESKTOP NAV - Spacing & Gold Underline */}
          <div className="hidden lg:flex items-center gap-12">
            {['Overview','Properties', 'Amenities', 'Location' ].map((item) => (
              <button 
                key={item}
              ...
              >
                <motion.span 
                  ...
                  className="text-[10px] uppercase tracking-[0.4em] font-semibold transition-opacity duration-300 group-hover:text-[#D4AF37]"
                >
                  {item}
                </motion.span>
            ...
            <button 
              ...
              className="group relative overflow-hidden border border-[#D4AF37] px-8 py-3 transition-all duration-500 hover:border-white"
            >
              <span className="relative z-10 text-[10px] uppercase tracking-[0.3em] font-bold text-[#D4AF37] group-hover:text-black transition-colors duration-500">
          ...
          <div className="lg:hidden z-[110]">
            <button onClick={() => setIsOpen(!isOpen)} className="p-2">
              <motion.div style={{ color: linkColor }}>
                {isOpen ? <X size={28} strokeWidth={1} className="text-white" /> : <Menu size={28} strokeWidth={1} />}
              </motion.div>
            </button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div 
              ...
              className="fixed inset-0 h-screen bg-black flex flex-col items-center justify-center gap-10"
            >
              <div className="absolute inset-0 opacity-20 pointer-events-none">
                <div className="h-full w-full bg-[radial-gradient(#D4AF37_1px,transparent_1px)] [background-size:40px_40px]" />
              </div>

              {['Overview', 'Highlights', 'Amenities', 'Location'].map((item, index) => (
                <motion.button 
                  ...
                  key={item}
                  onClick={() => scrollToSection(item.toLowerCase())} 
                  className="text-3xl font-light tracking-[0.5em] uppercase text-white hover:text-[#D4AF37] transition-all"
                >
                  {item}
                </motion.button>
              ))}
              
              <div className="bg-[#D4AF37] flex justify-center items-center">
  <motion.button 
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  ...
  >
    Request Brochure
      ...
      {/* FLOATING ACTION BUTTON - Minimalist Gold */}
      <a
        href={whatsappUrl}
        target="_blank"
