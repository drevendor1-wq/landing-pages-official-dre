  ...
  return (
    <section id="payment-plan" className="py-24 px-6 bg-[#F8FAFC]">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <button onClick={() => openEnquiryModal("Download Payment Plan")} className="group relative px-10 py-5 overflow-hidden rounded-full border border-green-500 transition-all duration-500">
            {/* Fill Effect */}
            
            {/* Button Content */}
            <span className="relative flex items-center gap-3 text-[#062C2D]">
              <Download size={18} className="group-hover:animate-bounce" />
              <span className="text-xs font-bold uppercase">Download Payment Plan</span>
            </span>
          </button>
        </motion.div>
      </div>
      <ContactModal isOpen={enquiryModalOpen} onClose={closeEnquiryModal} floorPlanTitle="Enquiry For: Greenz By Danube" buttonText={buttonText} />
    </section>
  );
};

const stepData = [
  { percent: "10%", label: "Down Payment", desc: "Secure your luxury beachfront Townhouse at Greenz with only a 10% down payment." },
  { percent: "60%", label: "During Construction", desc: "Enjoy flexible installments during the construction period for a stress-free investment journey." },
